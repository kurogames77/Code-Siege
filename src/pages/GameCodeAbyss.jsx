...
const GameCodeAbyss = () => {
    ...
    const languages = [
        'HTML', 'Python (Rec.)', 'C#', 'C++', 'JavaScript', 'PHP'
    ];
    ...
    const handleNextLevel = () => {
        ...
        if (currentFloor === 30) { // Max for Abyss
            navigate('/play');
            return;
        }

        const nextFloor = currentFloor + 1;
        navigate(`/gamecode-abyss/${nextFloor}/${towerId}`);
    };
    ...
    const setLanguage = (label) => {
        // Simple logic: allow any language change for now or restrict?
        // Prytody allowed Python only. Abyss text says "LEARN HTML".
        // I'll restricting if 'HTML' is the only option, but let's allow selection if it matches.
        if (label === 'HTML') {
